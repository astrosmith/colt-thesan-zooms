...
catalog_dir=/orcd/scratch/orcd/010/parth999/Thesan-Zooms-COLT
...
for job_set in "${job_sets[@]}"; do
  ...
  for run in "${runs[@]}"; do
    ...
    target_dir=${zoom_dir}/${group}/${run}/postprocessing/colt_tree/catalog
    mkdir -p $target_dir

    # Copy the files
    echo "cp -r ${catalog_dir}/${group}/${run}/catalog/* $target_dir"
    cp -r ${catalog_dir}/${group}/${run}/catalog/* $target_dir
  done
done

echo "Directories created and files copied successfully."
