...
for job_set in "${job_sets[@]}"; do
  ...
  for run in "${runs[@]}"; do
    ...
    target_dir=${zoom_dir}/${group}/${run}/postprocessing/colt_tree/${field}
    mkdir -p $target_dir

    # Move the files
    echo "mv ${colt_dir}/${group}/${run}/output_tree/${field}* $target_dir"
    mv ${colt_dir}/${group}/${run}/output_tree/${field}* $target_dir
  done
done
