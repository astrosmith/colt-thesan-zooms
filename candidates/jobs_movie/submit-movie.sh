...
# "job='B'; group='g39'; runs=('z4'); first_snap=0; last_snap=188"               # 0-188  ->  0-188  [2e9] long circ 95-188 [movie ?]
"job='C'; group='g205'; runs=('z4'); first_snap=0; last_snap=188"              # 0-188  ->  0-188  [2e9] long circ 128-188 [movie 1120]
# "job='D'; group='g578'; runs=('z4'); first_snap=0; last_snap=188"              # 0-188  ->  0-188  [2e9] long circ 170-188 [movie 1239]
# "job='E'; group='g1163'; runs=('z4'); first_snap=0; last_snap=188"             # 0-188  ->  0-188  [2e9]
# "job='F'; group='g5760'; runs=('z8'); first_snap=0; last_snap=188"             # 0-188  ->  0-188  [1e9] long circ 155-188 [movie 1197]
...
base_dir=/nfs/mvogelsblab002/Lab/Thesan-Zooms-COLT
...
for job_set in "${job_sets[@]}"; do
  ...
  for run in "${runs[@]}"; do
    ...
    cp config-movie*.yaml ${colt_dir}/.
    cp job-movie.sh ${colt_dir}/.

    cd ${colt_dir}
    pwd

    export group run
    sbatch -J ${job}_${run} job-movie.sh
  done
done
